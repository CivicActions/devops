# CivicActions Incident Response Plan (IRP)
...
This document outlines the CivicActions Incident Response Team's process for responding to internal or client security incidents and other system disruptions. It outlines roles and responsibilities during and after incidents, and it lays out the steps we'll take to resolve them.

Client specific modifications to this general policy may be made and will be documented in client-specific repositories.
...
* A CivicActions team member or client user (the *reporter*) notices and reports a potential security-related incident, notifying the CivicActions Incident Response Team:
  1. CivicActions Slack: [`#general`](https://civicactions.slack.com/messages/general/) using `@security`
  2. [CivicActions Incident Response Team contact sheet](https://docs.google.com/spreadsheets/d/1P9TePYm2Gkly8EjxCzA2EmlTjUIBypE7-CbCZrRN1EA/edit?usp=sharing)
...
* The IC forms a team (*responders*) to determine if the event is actually a confirmed incident, and if so [assesses the severity](#incident-severities).
  * The IC determines if a client Incident Response Plan should be activated.
  * The IC assesses whether to also activate the [contingency plan](contingency-plan.md).
* The IC posts an initial situation report ("sitrep") to Slack [`#general`](https://civicactions.slack.com/messages/general/)
...
* If appropriate to the situation, the IC coordinates with the Product Owner (PO) to notify affected teams/customers.
...
* If needed, the team can use a Google Hangout and/or Google Docs to share information that's not appropriate for Slack (PII, etc.).
...
At this point, the IC should write an initial situation report as a GitHub issue with label "Incident", summarizing what's going on, identifying the IC, and linking to the issue. Here's an example sitrep:
...
* Posted as a GitHub issue with label "Incident"
* Posted in [`general`](https://civicactions.slack.com/messages/general/) slack.

##### Comms at the Assess phase

Updates and real-time chat should continue as above (chat in Slack or Google Hangouts).
...
* Remediation may require service disruption. If it does, the team should proceed in a different way depending on the [severity](#incident-severities):
   ...
   * For Medium-severity incidents, the team should notify the CivicActions Information Security and Technology leads of the planned action, and help them assess the relative risk of disruption vs. security. If the leads are unavailable via Slack, they should be contacted using the phone numbers in their Slack profiles. The team should reach a collaborative decision on action, with a bias towards disruption. If they can't be reached within 1 hour, the team may take action without them.
...
*Note that CivicActions has few High Value Assets (HVAs) or Sensitive Personally Identifiable Information (SPII). As such, CivicActions incidents are generally expected to fall into the* **Low Severity** *bucket.*
